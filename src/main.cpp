...
#if defined(NDEBUG)
# error "Rajcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0f191edb5df1c4e2b468fba7191a338ef4eb8a6d964dabd7bfb818c5919ccbd6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rajcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rajcoin Signed Message:\n";
...
{
    // Rajcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Rajcoin: 50k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // Rajcoin: 0.5 days
static const int64 nTargetSpacing = 1 * 60; // Rajcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rajcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rajcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x27d9d8b5328deb3c46d305311d1c126bd3a426b39052af5174c7d54a5aece7fa");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Supreme court turns down Trumps Appeal in Dreamers case";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04392c8be842539a56dd6058da5c7f348805c2ef909306dbf4954a2441005cb7712f3e37c7fe78ffd185d8133c3687866d63aa445693be1efba1797e5814acabb7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519697961;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085878346;

        if (fTestNet)
        {
            block.nTime    = 1519697906;
            block.nNonce   = 385997419;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xce63f4c529337b0f7e76ea0552291af7695f98e8ef5ba1ef264125b4169e6424"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xb4, 0xda }; // Rajcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RajcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RajcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RajcoinMiner(CWallet *pwallet)
{
    printf("RajcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rajcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RajcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RajcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RajcoinMiner, pwallet));
}
